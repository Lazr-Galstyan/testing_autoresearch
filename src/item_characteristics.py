"""Point-in-time trading history for each ``stock_code``.

For every ``(stock_code, date)`` pair that appears in the corrected
first-transaction table, this module summarises how that product had traded
**strictly before that date**. The result is a lookup table that can be joined
back onto a transaction to answer "how established was this product at the moment
this customer bought it?".
...
Run from anywhere:
    python src/item_characteristics.py
"""
...
OUT_FILE = PROCESSED_DIR / 'item_characteristics.csv'
...
def build_item_characteristics(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Output:
        A frame with the columns in ``COLUMNS``, one row per
...
def main() -> None:
    """Build the item-history lookup table, validate it and write it out."""
